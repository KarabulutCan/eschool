...
{
    public class M240StudentController : Controller
    {
        ...
        #region RefundFutureDate
        [HttpGet]
        public IActionResult RefundFutureDate(int studentID, int userID)
        {
            ...
            if (studentTemp == null)
            {
                ...
                studentTemp.RefundedDueDate = DateTime.Now;
                _studentTempRepository.CreateStudentTemp(studentTemp);
            }
            else
            {
                if (studentTemp.RefundedDueDate == null) studentTemp.RefundedDueDate = DateTime.Now;
            ...
            var studentInstallment = _studentInstallmentRepository.GetStudentInstallmentID(schoolID, studentID);
        ...
        public IActionResult RefundFutureDate(StudentViewModel studentViewModel)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //getCode.SubTotal = studentViewModel.StudentTemp.SubTotal;

                getCode.RefundSW = "D";
                getCode.RefundedDueDate = studentViewModel.StudentTemp.RefundedDueDate;
                ...
                getCode.AboutRefund3 = studentViewModel.StudentTemp.AboutRefund2;
                ...
                var classrooms = classroom.ClassroomName;
                    ...
                    if (tempData != null)
                    {
                        foreach (var item in tempData)
                        {
                            ...
                            if (item.InstallmentAmount != item.PreviousPayment)
                            {
                                ...
                                AccountingCreate2(studentViewModel, school, item, sortOrder, period, classrooms, lastNumber);
                            }
                        }
                        if (total340 > 0)
                            AccountingCreate3(studentViewModel, school, sortOrder, period, classrooms, total340, lastNumber);
                    }
                ...
                log.TransactionID = _parameterRepository.GetParameterCategoryName("Öğrenci İade(Tarihli)").CategoryID;
                log.UserLogDate = DateTime.Now;
                string classroomName = _classroomRepository.GetClassroomID(studentViewModel.Student.ClassroomID).ClassroomName;
                ...
                log.UserLogDescription = studentViewModel.Student.FirstName + " " + studentViewModel.Student.LastName + " Öğrenci Kaydında (İleri Tarihli) İade İşlemleri Yapıldı, " + "Sınıfı:" + classroomName + ", Peşinat:" + cashPayment + "," + intstallment + " Ay Vadeli, " + "Toplam:" + subTotal + ", İade Edilen Tutar:" + refund;
        ...
        #region Accounting
        public void AccountingCreate2(StudentViewModel studentViewModel, SchoolInfo schoolInfo, TempData item, int sortOrder, string period, string classrooms, int lastNumber)
        {
        ...
        public void AccountingCreate4(StudentViewModel studentViewModel, SchoolInfo schoolInfo, int sortOrder, string period, string classrooms)
        {
            var accountingCode = new AccountCodes();
            accountingCode.Period = period;
            string studentNo = studentViewModel.Student.StudentNumber;
            ...
            var cashPayment = _studentTempRepository.GetStudentTemp(studentViewModel.SchoolID, period, studentViewModel.Student.StudentID).CashPayment;
            var bankName = _bankRepository.GetBank(studentViewModel.StudentTemp.BankID).BankName;
            var exp1 = " ";
            var exp2 = " ";
            decimal total340 = 0;
            ...
            if (studentViewModel.StudentTemp.RefundAmount1 > 0)
            {
                ...
                accounting.Explanation = classrooms + " " + studentViewModel.Student.FirstName + " " + studentViewModel.Student.LastName + " " + paymentNo + exp1 + DateTime.Today.ToString("dd.MM.yyyy") + exp2;
            ...
            accounting.Explanation = classrooms + " " + studentViewModel.Student.FirstName + " " + studentViewModel.Student.LastName + " " + paymentNo + exp1 + exp5;
            accounting.Debt = total340;
            accounting.Credit = 0;
            accounting.SortOrder = sortOrder;
            accounting.IsTransaction = false;
        ...
        #region Others
        [HttpPost]
        [Route("M240Student/AboutRefund/{schoolID}/{aboutRefund}/{period}/{studentid}")]
        public IActionResult AboutRefund(int schoolID, string aboutRefund, string period, int studentid)
        {
            if (ModelState.IsValid)
            {
                ...
                getCode.AboutRefund3 = aboutRefund;
                _studentTempRepository.UpdateStudentTemp(getCode);
            }
            return View();
        }

        [Route("M240Student/SchoolDebtDataRead/{userID}/{studentid}")]
        public IActionResult SchoolDebtDataRead(int userID, int studentid)
        {
            ...
            foreach (var item in fee)
            {
                ...
                if (debt != null)
                {
                    //studentDebtViewModel.StudentID = debt.StudentID;
        ...
        [HttpPost]
        [Route("M240Student/SchoolDebtDataUpdate/{strResult}")]
        ...
        //Installment
        [Route("M240Student/InstallmentDataRead/{userID}/{studentid}")]
        ...
        [Route("M240Student/PaymentsDataRead/{userID}/{studentID}")]
        ...
        [Route("M240Student/SerialNumbersDataRead/{userID}")]
        public IActionResult SerialNumbersDataRead(int userID)
        {
            ...
            {
                //if (item.CategoryName != "Banka" && item.CategoryName != "Elden")
                //{
        ...
        [Route("M240Student/ClassroomCombo/{userID}")]
        ...
        [Route("M240Student/AccountCodes/{period}")]
